...
class AudioKeyManager(PacketsReceiver, Closeable):
    ...
    def get_audio_key(self,
        ...
        """Retrieve audio key from external Audio Key server instead of Spotify directly.

        The server endpoint and behaviour are configured via environment variables:

          AUDIO_KEY_SERVER_URL  (default: http://127.0.0.1:8765/get_key)
        ...
        and the expected response is:
          {"key": "<hex>"}
        """
        server_url = os.getenv("AUDIO_KEY_SERVER_URL", "http://127.0.0.1:8765/get_key")

        # SpotiClub client credentials used for authenticating against the audio key server.
        # These can be provided via environment variables or a small JSON file next to the
        # Zotify project root, `spoticlub_api.json`, with the structure:
        #   {"ftpUser": "spoticlub-...", "password": "..."}
        ftp_user = ""
        ftp_password = ""

        if not ftp_user or not ftp_password:
            try:
                cfg_path = Path.cwd() / "spoticlub_api.json"
                if cfg_path.is_file():
                    with open(cfg_path, "r", encoding="utf-8") as f:
                        cfg = json.load(f)
                    ftp_user = ftp_user or cfg.get("ftpUser")
                    ftp_password = ftp_password or cfg.get("password")
            except Exception:
                # If loading fails we simply fall back to env-only behaviour.
                pass

        if not ftp_user or not ftp_password:
            raise RuntimeError(
                "Missing SpotiClub credentials: please set SPOTICLUB_FTP_USER/SPOTICLUB_FTP_PASSWORD "
                "or create spoticlub_client.json with ftpUser/password."
            )
        ...
        while True:
            ...
            try:
                ...
                if resp.status_code != 200:
                    raise RuntimeError(f"Audio key server returned {resp.status_code}: {resp.text}")
                data = resp.json()
                key_hex = data.get("key")
                if not isinstance(key_hex, str):
                    raise RuntimeError("Audio key server response missing 'key'")
                key_bytes = util.hex_to_bytes(key_hex)
                if len(key_bytes) != 16:
                    raise RuntimeError("Audio key must be 16 bytes long")
                return key_bytes
            except Exception as exc:  # noqa: BLE001
                last_err = exc
                self.logger.warning("Audio key server error (try %d): %s", tries, exc)
                if not retry or tries >= 3:
                    break
                time.sleep(5)

        raise RuntimeError(
            "Failed fetching audio key from server for gid: {}, fileId: {} (last error: {})".format(
